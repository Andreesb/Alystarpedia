...
def process_data(data):
    ...
    def parse_boolean(value):
        return value.strip() in ['✓', 'Yes']  # Solo estos son True, los demás son False
...
async def save_boss_data(name, wiki_url, wiki_data, db_path="data/databases/bontar_data.db"):
    img_url = f"https://tibia.fandom.com/wiki/Special:Redirect/file/{quote(name)}.gif"

    query = """
    INSERT INTO bosses (
        name, category, wiki_url, image_url, hp, exp, est_dmg, classification, 
        spawn_type, illusionable, pushable, pushes, paralysable, sense_invis, walks_around, notes, 
        abilities, resistance, location, behaviour, strategy, loot
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """

    values = (
        name,
        wiki_data.get("Bosstiary Properties", {}).get("Category", "Unknown"),
        wiki_url,
        img_url,
...
def scrape_bosses_data(name, wiki_url):
    item_data = {}
    item_data['name'] = name
...
async def main():
    ...
    with open("bosses_names.txt", "r") as file:
    ...
    for name in bosses_name:
        ...
        wiki_data = scrape_bosses_data(name, wiki_url)

        if wiki_data:
            await save_boss_data(name, wiki_url, wiki_data)
        else:
            error_message = f"{name}\n"
            async with aiofiles.open("errors.txt", "a") as f:
